  ...
  function runOnboarding(done) {
    ...
    function stepWelcome() {
      ob.innerHTML =
        ...
        '</div>';
      U.qs('#ob-start', ob).addEventListener('click', stepProfile);
    }

    const THEMES = [
      { id: '', label: 'Groen', color: '#2e7d51' },
      { id: 'gold', label: 'Goud', color: '#d4903b' },
      { id: 'blue', label: 'Blauw', color: '#4f8fd4' },
      { id: 'purple', label: 'Paars', color: '#9b6fd4' },
      { id: 'red', label: 'Rood', color: '#d4695f' }
    ];
    ...
    function stepProfile() {
      ob.innerHTML =
        '<div class="ob-card">' +
        '<h1>Jouw profiel</h1>' +
        '<p class="ob-text">Hoe heet je? En kies eventueel een themakleur.</p>' +
        '<input class="ob-input" id="ob-name" type="text" maxlength="24" placeholder="Jouw naam" autocomplete="off">' +
        '<div class="ob-swatches" role="radiogroup" aria-label="Themakleur">' +
        THEMES.map((t, i) =>
          '<button type="button" class="ob-swatch' + (i === 0 ? ' active' : '') + '" data-theme="' + t.id + '" style="--sw:' + t.color + '" aria-label="' + t.label + '"></button>'
        ).join('') +
        '</div>' +
        '<p class="ob-note">' + Icons.shieldCheck + '<span>Alles blijft lokaal op dit toestel.</span></p>' +
        '<button class="btn btn-gold btn-block" id="ob-next">Volgende</button>' +
        '</div>';
      let chosen = '';
      U.qsa('.ob-swatch', ob).forEach((s) =>
        s.addEventListener('click', () => {
          chosen = s.getAttribute('data-theme');
          U.qsa('.ob-swatch', ob).forEach((x) => x.classList.toggle('active', x === s));
        })
      );
      U.qs('#ob-name', ob).focus();
      U.qs('#ob-next', ob).addEventListener('click', () => {
        const nm = (U.qs('#ob-name', ob).value || '').trim() || 'Liam';
        stepPin({ name: nm, theme: chosen });
      });
    }

    function stepPin(profile) {
      ob.innerHTML =
        ...
        onSubmit: (pin) => {
          if (!first) {
            ...
            configure(first).then(async () => {
              ...
              await DB.setSetting('userName', profile.name);
              await DB.setSetting('themeColor', profile.theme);
